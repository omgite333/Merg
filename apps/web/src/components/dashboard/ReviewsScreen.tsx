...
export function ReviewsScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all([getReviews(page), getDashboard()])
      .then(([nextReviews, nextDashboard]) => {
        if (!cancelled) {
          setReviewsData(nextReviews);
          setDashboardData(nextDashboard);
          setError(null);
    ...
    <div className="space-y-7">
      <section className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          ...
          <h1 className="mt-2 text-[32px] font-semibold leading-none tracking-[-0.06em] text-[#20201e] dark:text-white sm:text-[38px]">Every review, in one place.</h1>
          <p className="mt-3 max-w-xl text-[14px] leading-6 text-[#73736e] dark:text-[#a1a1aa]">Inspect completed, active, and failed review sessions without searching through GitHub notifications.</p>
        </div>
      ...
      <section className="rounded-2xl border border-[#e5e5e0] bg-white dark:border-white/10 dark:bg-[#14141a] p-4 shadow-[0_8px_24px_rgba(23,23,23,0.035)] sm:p-5">
        <div className="grid gap-3 lg:grid-cols-[minmax(180px,0.9fr)_minmax(180px,0.8fr)_minmax(0,1.3fr)]">
          <label className="relative block">
            ...
            <select value={status} onChange={(event) => setStatus(event.target.value as "ALL" | ReviewStatus)} className="h-10 w-full appearance-none rounded-xl border border-[#e3e3dd] bg-[#fcfcfa] dark:bg-[#101016] pl-9 pr-3 text-[12px] font-medium text-[#4d4d47] dark:text-[#d3d3d5] outline-none transition-colors focus:border-[#8fb2fa]">
          ...
          <label className="relative block">
            ...
            <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search repository or PR number" className="h-10 w-full rounded-xl border border-[#e3e3dd] bg-[#fcfcfa] dark:bg-[#101016] pl-9 pr-3 text-[12px] font-medium text-[#4d4d47] dark:text-[#d3d3d5] outline-none placeholder:text-[#aaa9a3] dark:text-[#a9a9b1] transition-colors focus:border-[#8fb2fa]" />
          </label>
        </div>
        <p className="mt-3 text-[11px] text-[#92928c] dark:text-[#b8b8c0]">Filters apply to the reviews on this page. Server-side filtering can be added once review volume grows.</p>
      </section>
      ...
      <section className="overflow-hidden rounded-2xl border border-[#e5e5e0] bg-white dark:border-white/10 dark:bg-[#14141a] shadow-[0_8px_24px_rgba(23,23,23,0.035)]">
        <div className="flex flex-col justify-between gap-2 border-b border-[#ecece7] px-5 py-4 sm:flex-row sm:items-center sm:px-6">
          <div>
            ...
            <p className="mt-1 text-[12px] text-[#83837d] dark:text-[#b8b8c0]">{pagination?.total ?? 0} total {pagination?.total === 1 ? "review" : "reviews"} · newest first</p>
          </div>
          <p className="text-[12px] font-medium text-[#777771] dark:text-[#a1a1aa]">{filteredReviews.length} visible on this page</p>
        </div>

        {filteredReviews.length ? (
          <div className="divide-y divide-[#efefeb] dark:divide-white/10">
            {filteredReviews.map((review) => (
              <article key={review.id} className="grid gap-4 px-5 py-5 transition-colors hover:bg-[#fcfcfa] dark:bg-[#101016] dark:hover:bg-white/5 md:grid-cols-[minmax(0,1.3fr)_auto_auto] md:items-center md:px-6">
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                      <p className="truncate text-[13px] font-semibold text-[#33332f] dark:text-[#e2e2e1]">{review.repository.fullName}</p>
                      <span className="text-[12px] text-[#8b8b85] dark:text-[#b8b8c0]">PR #{review.prNumber}</span>
                    </div>
                    <p className="mt-1 text-[12px] text-[#85857f] dark:text-[#9a9aa3]">Opened {formatRelativeTime(review.createdAt)} · {review.filesReviewed} {review.filesReviewed === 1 ? "file" : "files"} reviewed · {review.totalComments} {review.totalComments === 1 ? "finding" : "findings"}</p>
                  </div>
                </div>
          ...
          <div className="px-6 py-16 text-center">
            ...
            <p className="mt-3 text-[13px] font-medium text-[#4d4d48] dark:text-[#d3d3d5]">No reviews match these filters.</p>
